...
@Repository("memberDao") // servlet-context.xml에 dao 클래스로 자동 등록하는 어노테이션임 (id 속성의 아이디 지정)
public class MemberDao {
	// 마이바티스 매퍼 파일에 실핼시킬 쿼리문(sql 문) 별도로 작성함 
	// root-context.xml에 설정된 마이바티스 세션 객체를 연결해서 사용함 
	
	@Autowired // root-context.xml에서 생성한 객체를 자동 연결하는 어노테이션임 
