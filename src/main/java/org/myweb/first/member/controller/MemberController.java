...
@Controller // servlet-context.xml에 Controller로 자동 등록되는 어노테이션임
public class MemberController {
	...
	// spring framework에서는 부모 인터페이스 타입으로 레퍼런스 선언함(다형성 사용함)
	...
	@RequestMapping("loginPage.do") // 뷰페이지에서 사용하는 요청이름 등록용 어노테이션임, Mapping Handler가 관리함
	public String moveLoginPage() {
		return "member/loginPage"; // 뷰 리졸버로 리턴됨 >> 뷰리졸버가 지정된 폴더에서 해당 jsp 파일 찾아서 클라이언트로 전송함
	}
	...
	// 요청 받아서 서비스 모델 쪽으로 전달정보 넘기고 결과 받는 메소드 ----------------------------------------
...
//
//		// 3. 모델측 서비스 클래스의 메소드를 실행시키고 결과 받기
//
...
//			// session.setMaxInactiveInterval(30 * 60); // 초 단위로 설정함 : 30분동안 요청이 없으면 세션 객체
//			// 무효화됨
//
//			// 세션 객체 안에 로그인한 회원 정보를 기록 저장시킬 수 있음 (뷰 페이지에서 꺼내서 사용할 경우)
...
//	} //login.do
	...
	// input 태그의 name 속성의 이름을 dto의 필드명 (property)과 일치시키면,
...
//			return "common/main";
//		}else { // 로그인 실패시
//			// 스프링에서는 서블릿의 RequestDispatcher를 대신하는 Model 클래스를 제공함 
//			// Model이 제공되는 이유 : 스프링에서는 포워딩 못함 ( 뷰 리졸버로 리턴되기 때문 )
//			// 뷰로 같이 전달할 정보를 뷰와 함께 내보내기 위해 사용함
//			// model에 뷰로 내보낼 정보를 저장할 때 : addAttribute ("이름", 객체) == request.setAttribute("이름", 객체)
//			model.addAttribute("message", "로그인 실패! 아이디 혹은 암호를 다시 확인하세요. \n 또는 로그인 제한 회원입니다. 관리자에게 문의하세요");
//			return "common/error";
//		}	
//	}
	...
	public String loginMethod(Member member, HttpSession session, SessionStatus status, Model model) {
		...
		// 암호화된 패스워드와 일치한지는 select 해온 값으로 비교함
		// 로그인 요청은 회원의 아이디로 먼저 회원 정보를 조회해 옴 >> 조회해온 정보에서 암호를 꺼내서 비교 확인함
		Member loginUser = memberService.selectMember(member.getUserId());

		// 조회해온 회원 정보가 있고, 암호화된 패스워드와 로그인 시 전송온 패스워드가 일치한지 비교함
		// matches(전달받은 글자암호, 암호화된 패스워드) 사용함, true이면 일치를 의미함
		if (loginUser != null && this.bcryptPasswordEncoder.matches(member.getUserPwd(), loginUser.getUserPwd())) {
			...
			// Model이 제공되는 이유 : 스프링에서는 포워딩 못함 ( 뷰 리졸버로 리턴되기 때문 )
			// 뷰로 같이 전달할 정보를 뷰와 함께 내보내기 위해 사용함
			// model에 뷰로 내보낼 정보를 저장할 때 : addAttribute ("이름", 객체) ==
	...
	// 로그아웃 처리용 메소드
	// 요청에 대한 전송 방식이 get 이면, @RequestMapping에 method 속성 생략해도 됨
	// method 속성을 생략하면, value 속성도 표기를 생략해도 됨
	@RequestMapping("logout.do")
	public String logoutMethod(HttpServletRequest request, Model model) {
		HttpSession session = request.getSession(false);
		// 해당 세션 객체가 있으면 리턴 받고, 없으면 null 리턴
		if (session != null) {
			...
			// 에러 메세지를 error.jsp로 내보내기함 >> 서블릿이라면 RequestDispatcher의 forward() 사용함
			// Spring에서는 Model 클래스를 제공함 >> request.setAttribute("에러메세지");
			// Model의 addAttribute() 메소드 사용함
	...
	// 클라이언트 쪽에서 입력한 userId를 전송보내면, request 객체에 저장되어서 전송옴
	// 전송온 값 꺼내기 : String 변수 = request.getParameter("전송용이름");
	// 위의 코드를 처리해주는 어노테이션을 메소드 매개변수 위치에 사용할 수 있음
...
//	public void dupCheckUserIdMethod(@RequestParam("userId") String userId, HttpServletResponse response) {
//		// ajax 요청시 결과 응답하는 방식 : @ResponseBody에 담아서 보내는 방식, 별도의 출력 스트링을 만들어서 내보내기 
//		// 방식2, 별도의 출력스트림 연결해서 문자열 내보내기 방식 사용함 (반환자료형이 void임)
//		
//		int result = memberService.selectCheckedId(userId);
...
//		}
//		
//		// response를 이용해서 클라이언트와 출력스트림을 열어서 문자열값 내보내기할 것 
...
//	}
	...
	public String dupCheckUserIdMethod(@RequestParam("userId") String userId) {
		// 방식1. ResponseBody에 담아서 문자열 내보내기 방식 사용함 (반환자료형이 String임)
		int result = memberService.selectCheckedId(userId);
...
//		logger.info("암호화 패스워드 : " + encodePwd);
...
//	}
	...
	// 업로드 되는 파일은 따로 전송받음 >> multipart 방식으로 전송옴 >> 스프링이 제공하는 MultipartFile 클래스 사용해서
	// 받음
	// 비밀번호 암호화 처리 기능 추가
	@RequestMapping(value = "enroll.do", method = RequestMethod.POST)
	public String memberInsertMethod(Member member, Model model, HttpServletRequest request,
		...
		// 서버 엔진이 구동하는 웹 애플리케이션(Context)의 루트(webapp)아래의 "resources/photoFiles" 까지의 경로
		// 정보를 저장함
		logger.info("savePath : " + savePath);

		// 첨부파일이 있다면 지정 폴더에 저장처리 -----------------------------------------
		if (!mfile.isEmpty()) {
			// 전송온 파일 이름 추출함
			String fileName = mfile.getOriginalFilename();
			// 여러 회원이 업로드한 사진 팡일명이 중복될 경우를 대비해서 저장파일명 이름 바꾸기함
			// 바꿀 파일 이름은 개발자가 정함
			// userId가 기본키(PrimaryKey)이므로 중복이 안됨 >> userId_filename.확장자 형태로 정해봄
	...
	public String memberUpdateMethod(Member member, Model model, HttpServletRequest request,
		...
		if (!mfile.isEmpty()) {
			...
			// 기존 파일명을 member에 다시 저장함
			member.setPhotoFileName(member.getUserId() + "_" + originalFileName);
		}

		// 서비스 모델의 메소드 실행 요청하고 결과받기
		if (memberService.updateMember(member) > 0) { // 회원 정보 수정 성공
			// 컨트롤러 매소드에서 다른 컨트롤러 매소드를 실행시킬 경우 
			return "redirect:main.do";
		} else { // 회원 정보 수정 실패
			model.addAttribute("message", "회원 정보 수정 실패! 다시 확인해 주세요.");
			return "common/error";
		}
	}
	...
	public ModelAndView memberSearchMethod(HttpServletRequest request, ModelAndView mv) {
		...
		if(list != null && list.size() > 0) {
			...
			mv.addObject("message", "회원 관리 검색 결과가 존재하지않습니다.");
			mv.setViewName("common/error");
		}
		return mv;
	}
