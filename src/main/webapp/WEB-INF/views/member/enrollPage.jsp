<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
 <%-- <%@ taglib url ="https://jakarta.ee/jsp/jstl/core" prefix="c"  %> --%>
...
<script type="text/javascript">
 function dupIdCheck(){
	 ...
	 $.ajax({
		 url: 'idchk.do',
	 	type: 'post',
	 	data: { userId: $('#userId').val() }, <%-- 전송 보낼 값 지정 속성임 { 전송이름: 전송값 } --%>
	 	<%-- 응답오는 값이 문자열이면, dataType 옵션으 생략해도 됨 --%>
 ...
 function validate() {
	 // 서버측으로 전송보내기 전에 입력값들이 유효한지 검사하는 함수 
 ...
 // 제이쿼리 : $(function(){ ... }); 축양형이고, jQuery(document).ready(function(){ ... }); 과 같음
 window.onload = function(){
	...
	photofile.addEventListener('change', function(event){
		...
		// photo.src = '서버측에 있는 이미지 파일의 상대 | 절대 경로/' + file.name; (서버측에 있는 이미지 파일임)
		...
		reader.readAsDataURL(file); // 읽어서 img에 적용 출력함 
...
<%-- 속성 url은 브라우저에 표시되는 페이지 경로에 대한 url임 
	기본 웰컴 페이지 url이 http://localhost:8080/first 임
	url 속성에 작성된 값을 기본 url에 합쳐서 표현하면 
	전체 url이 http://localhost:8080/first/WEB-INF/views/common/menubar.jsp 작성하거나 
	또는 기본 url은 생략해도 되므로 
...
<br>
<%-- form에서 파일이 첨부되어서 전송이 될 경우에는, 반드시 enctype="multipart/form-data" 속성을 추가해야 함 --%>
...
<table id="outer" align="center" width="700" cellspacing="5" cellpadding="0">
		...
		<%-- input 태그의 name 속성의 이름은 member.dto.Member 클래스의 property명과 같게 함 --%>
		...
		<%-- 첨부된 사진 미리보기가 되도록하려면, 사진 미리보기용 영역 지정 : 서버로 전송 안됨 --%>
		...
		<%-- 첨부파일의 name은 필드명(property)와 별개로 지정함 
			파일 업로드 실패시 파일명만 문자열로 command 객체(dto)에 저장되지 않게 하기 위함 
		--%>
		</td></tr>
		<tr><th>*암호</th>
