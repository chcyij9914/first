...
<c:set var="array" scope="request"> <%-- scope="request" array 변수를 request 객체에 저장함
...
<c:remove var="total" scope="page" /> <%-- page 객체에 저장된 total 변수 제거함, scope의 page (기본값) --%>
total 변수 값 : ${ total } <br>
<hr>

<h4>c:out : 출력기능 제공 태그</h4>
그대로 출력 : <c:out value="<h2>태그로 출력하기</h2>"  escapeXml="true" /> <br>
태그 해석해서 출력 : <c:out value="<h2>태그로 출력하기</h2>" escapeXml="false"/> <br>
전송 온 값 출력 : <c:out value="${ param.name }"  default="홍길동" /> <br>
<%-- ${ param.전송온이름 } == request.getParameter("전송온이름")과 같음 
		실행 테스트시에는 브라우저 주소 표시줄에 기존 url 뒤에 직접 입력함 : 
...
<h4>c:forEach 태그 : for 문, for each문 처리를 하는 태그</h4>
...
<!-- <a href="moveFmt.do?no=123">testFmt 페이지로 이동</a> <br> -->
...
별도로 작성된 jsp 또는 html 파일을 원하는 위치에 가져가 표시할 수 있음  (포함시키다) <br>
전체 뷰 페이지에서 반복되는 영역 또는 메뉴바, 햄버거 메뉴등을 표시할 때 주로 사용됨 <br>
<hr>
<c:import url="/WEB-INF/views/common/footer.jsp" />
<%-- url="http://localhost:8080/first/WEB-INF/views/common/footer.jsp" 
		기본 월컴 url : http://localhost:8080/first 생략이 가능
--%>
<hr>
...
<c:catch var="e">
	...
	나눈 결과 : ${ num3 / num } <br> <%-- Exception 발생 - e 변수가 예외정보를 저장함 --%>
</c:catch>
<c:if test="${ e != null }">
